...
class AES
{
  ...
  public function __construct($strength = self::AES128)
  {
    ...
    // $this->_cipher = new AES(AES::AES128);
    // $this->_strenght = AES::AES128;
  }
  /**
   * Class destructor
   */
  ...
  public function pharse($colour, $string)
  {
    ...
    if ($colour == 'red') {
      $output = openssl_encrypt($string, METHODBOY, $key, 0, $iv);
      $output = base64_encode($output);
      $output = str_replace("=", "", $output);
    } else if ($colour == 'blue') {
      $output = openssl_decrypt(base64_decode($string . "="), METHODBOY, $key, 0, $iv);
    }
    return $output;
  }
  ...
  public function redmoon($content)
  {
    $content = $content + 0.1;
    $key = $this->_generateKey(SHOULDIBEAKEY);
    $input = str_split($this->stringToHex(strval($content)), self::BYTE_LIMIT * 2);
    $output = '';
    foreach ($input as $chunk)
      $output .= $this->encrypt($chunk, $key);
    $x = $this->hexToString($output);
    $ret = $this->pharse("red", $x);
    return $ret;
  }

  public function bluesun($cryptext)
  {
    $y = $this->pharse("blue", $cryptext);
    $key = $this->_generateKey(SHOULDIBEAKEY);
    $input = str_split($this->stringToHex($y), self::BYTE_LIMIT * 2);
    $output = '';
    foreach ($input as $chunk)
      $output .= $this->decrypt($chunk, $key);
    $ret = floatval($this->hexToString($output) - 0.1);
